...
/**
 * The pitch prompt, and the event it is about.
 *
 * Extracted from the route so both surfaces can use it: the web app renders it
 * and calls Anthropic with the deployment's credentials, while an MCP client
 * renders the same prompt and judges it on the person's own subscription. The
 * wording is the thing that must not fork — a pitch written from a paraphrased
 * brief is a different pitch.
 *
 * Returns null when the event does not exist, so the caller decides the status
 * code.
 */
export async function buildPitchPrompt(
  userId: string,
  eventId: string,
): Promise<{ prompt: string; event: { id: string; title: string } } | null> {
...
/**
 * Save a drafted application to THIS speaker's own row.
 *
 * The draft is written in their voice, from their brief — it belongs on their
 * opportunity, not on the shared event where it would be served to everyone.
 */
export async function savePitchDraft(userId: string, eventId: string, pitchDraft: string) {
  const updated = await db.eventOpportunity.upsert({
    where: { userId_eventId: { userId, eventId } },
    create: { userId, eventId, pitchDraft },
    update: { pitchDraft },
  });
  return updated.pitchDraft;
}
