...
export default function TopBar({ name, email, role, mustChangePassword, isGuest }: TopBarProps) {
  ...
  return (
    <div className="flex-shrink-0">
      {mustChangePassword && (
        <div className="bg-[#FF8067]/10 border-b border-[#FF8067]/20 px-4 py-2 text-sm text-[#FF8067] flex items-center gap-2">
          ...
          <span>
            ...
            <a href="/change-password" className="underline font-semibold hover:text-[#FF8067]/80 transition-colors">
              Change it now
            </a>{" "}
            for security.
          </span>
      ...
      <header className="h-12 bg-[#090B0B] border-b border-white/[0.07] flex items-center justify-between px-4 md:px-5">
        ...
        <div className="flex items-center gap-3">
          ...
          {isGuest && (
            <a
              href="/login"
              className="font-mono text-[10px] uppercase tracking-[0.08em] text-[#BC7CFF] hover:text-[#CA96FF] px-3 py-1.5 rounded border border-[#BC7CFF]/30 hover:border-[#BC7CFF]/50 transition-all"
            >
              Log in to edit
            </a>
          )}
